...
type UploadedFiles = Express.Multer.File[] | Record<string, Express.Multer.File[]>;

const getFilesForField = (
...
export const listAssets = async (req: Request, res: Response) => {
    try {
        const requestedType = queryToOptionalString(req.query.assetType);
...
export const uploadAsset = async (req: Request, res: Response) => {
    try {
        ...
        const assetFiles = getFilesForField(files, assetType);
...
export const updateAsset = async (req: Request, res: Response) => {
    try {
        ...
        const files = (req as Request & { files?: UploadedFiles }).files;
        const newFile = getFilesForField(files, assetType)[0];
        if (!newFile?.filename) {
            return res.status(400).json({
                success: false,
