  ...
  return (
    <View className="flex-1 bg-background" style={{ height: Platform.OS === 'web' ? ('100vh' as any) : '100%' }}>
      ...
      <KeyboardAvoidingView behavior={Platform.OS === 'ios' ? 'padding' : 'height'} className="flex-1" keyboardVerticalOffset={Platform.OS === 'ios' ? 90 : 0}>
        <ScrollView className="flex-1 p-4 md:p-6" showsVerticalScrollIndicator={false}>
          ...
          <Card className="mb-6 p-4 md:p-6">
            ...
            <View className="flex-row flex-wrap -mx-2">
              <View className="w-full md:w-1/2 px-2 mb-4"><Input label="Phone Number *" keyboardType="phone-pad" value={customer.phone} onChangeText={(v) => setCustomer({ ...customer, phone: v })} placeholder="Enter phone number" /></View>
              <View className="w-full md:w-1/2 px-2 mb-4"><Input label="Name" value={customer.name} onChangeText={(v) => setCustomer({ ...customer, name: v })} placeholder="Customer name" /></View>
              <View className="w-full md:w-1/2 px-2 mb-4"><Input label="Email" keyboardType="email-address" value={customer.email} onChangeText={(v) => setCustomer({ ...customer, email: v })} placeholder="Email address" /></View>
              <View className="w-full md:w-1/2 px-2 mb-4"><Input label="Address" value={customer.address} onChangeText={(v) => setCustomer({ ...customer, address: v })} placeholder="Street, City, State" /></View>
            </View>
          </Card>
                ...
                {cart.map((item) => (
                  <View key={item.id} className="flex-row items-center px-3 py-2 border-b border-border/50">
                    ...
                    <View className="w-16 items-center">
                      <Input value={String(item.qty)} onChangeText={(v) => updateQty(item.id, v)} keyboardType="numeric" className="h-8 w-12 text-center text-xs p-0" />
                    </View>
          ...
          <Card className="mb-10 p-4 md:p-6">
            ...
            <View className="flex-row flex-wrap -mx-2">
              <View className="w-1/2 px-2 mb-4"><Input label="Discount (₹)" keyboardType="numeric" value={discount} onChangeText={setDiscount} /></View>
              <View className="w-1/2 px-2 mb-4"><Input label="Tax (₹)" keyboardType="numeric" value={tax} onChangeText={setTax} /></View>
            </View>
        ...
        {/* Sticky Total Footer */}
        <View className="bg-card border-t border-border flex-row items-center justify-between px-6 pt-4" style={{ paddingBottom: Math.max(insets.bottom, 24), shadowColor: "#000", shadowOffset: { width: 0, height: -4 }, shadowOpacity: 0.1, shadowRadius: 10, elevation: 20 }}>
