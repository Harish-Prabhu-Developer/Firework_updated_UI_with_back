...
import { Pencil, Trash2, User, Phone, Mail, MapPin } from "lucide-react-native";
...
const initialData: Customer[] = [
  { id: "1", name: "Rajesh Kumar", phone: "9876543210", address: "123 Main St, Chennai", gst: "33AAACR1718Q1ZB", email: "rajesh@email.com", status: "Active" },
  { id: "2", name: "Priya Sharma", phone: "9876543211", address: "456 Park Ave, Mumbai", gst: "", email: "priya@email.com", status: "Active" },
];
...
export default function Customers() {
  ...
  const handleSubmit = () => {
    ...
    if (editItem) {
      setData(data.map((i) => i.id === editItem.id ? { ...i, ...form } as Customer : i));
    } else {
      setData([...data, { ...form, id: String(Date.now()) } as Customer]);
    }
    setFormOpen(false);
  };
      ...
      <AdaptiveTable
        ...
        renderCardHeader={(i) => (
          <View className="flex-row items-center gap-3">
            <View className="h-10 w-10 rounded-full bg-primary/10 items-center justify-center border border-primary/20">
        ...
        renderCardBody={(i) => (
          <View className="gap-3">
            <View className="bg-muted/50 p-3 rounded-xl border border-border/50 gap-2">
              ...
              <View className="flex-row items-start gap-2 pt-1 border-t border-border/30">
                <MapPin size={12} color="#64748b" className="mt-0.5" />
                <Text className="text-xs text-muted-foreground flex-1" numberOfLines={2}>{i.address}</Text>
              </View>
            </View>
          </View>
        )}
        ...
        <ScrollView className="max-h-[70vh]">
          <View className="gap-5 pb-10 p-1">
            ...
            <Textarea
              ...
              placeholder="Complete office or home address"
            />
          </View>
        </ScrollView>
      </Dialog>
