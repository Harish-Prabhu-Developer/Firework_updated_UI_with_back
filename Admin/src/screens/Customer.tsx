...
export default function Customer() {
  ...
  // Used ONLY in card view for address — wrapping is intentional there.
  // Do NOT apply to table cell text; use numberOfLines + ellipsizeMode there instead.
  ...
  // minWidth:0 forces the flex child to actually shrink so numberOfLines kicks in.
  const truncStyle = {
    flexShrink: 1,
    minWidth: 0,
    overflow: 'hidden',
  ...
  const columns: Column<Customer>[] = [
    {
      ...
      sortable: true,
      render: (c) => (
        <View className="flex-row items-center gap-2" style={{ minWidth: 0, maxWidth: '100%' }}>
          <View
            className="w-8 h-8 rounded-full bg-primary/10 items-center justify-center"
            style={{ flexShrink: 0 }}
          >
            <Text className="text-xs font-black text-primary" style={{ fontFamily: Fonts.body }}>
              {c.name[0]?.toUpperCase()}
            </Text>
          </View>
          {/* FIX: was wrapTextStyle (flexWrap:'wrap') — long names wrapped instead of truncating */}
          <Text
    ...
    },
    {
      key: 'phone',
      label: 'Phone',
      width: 150, // bumped from 140 — mono "+91 98942 09220" needs the room
      render: (c) => (
        // FIX: was wrapTextStyle which has flexWrap:'wrap' + wordBreak:'break-word' +
        // overflowWrap:'anywhere' — these web CSS properties caused the phone number
        // to snap to two lines, making the row taller and misaligning every other cell.
        // Phone numbers must never wrap; use numberOfLines={1} + ellipsizeMode instead.
        <Text
    ...
    },
    {
      ...
      width: 200,
      render: (c) => (
        // Address allows 2 lines in the table — wrapping is intentional here
        <Text
    ...
    },
    {
      ...
      width: 120,
      render: (c) => (
        <Text className="text-xs text-muted-foreground" style={{ fontFamily: Fonts.body }}>
      ...
      render: (c) => (
        <View className="flex-row gap-1">
          <PermissionGuard module="Customers" action="Update">
            <TouchableOpacity onPress={() => openEdit(c)} className="w-8 h-8 rounded-lg bg-primary/10 items-center justify-center">
              <Pencil size={14} color={colors.primary} />
            </TouchableOpacity>
          </PermissionGuard>
          <PermissionGuard module="Customers" action="Delete">
            <TouchableOpacity onPress={() => setDeleteId(c.id)} className="w-8 h-8 rounded-lg bg-destructive/10 items-center justify-center">
    ...
    <View style={globalStyles.card}>
      <View className="flex-row items-center gap-3 mb-4">
        <View className="w-12 h-12 rounded-full bg-primary/10 items-center justify-center">
          <Text className="text-lg font-black text-primary" style={{ fontFamily: Fonts.display }}>
            {c.name[0]?.toUpperCase()}
          </Text>
        </View>
      ...
      {c.address && (
        <View className="flex-row items-start gap-2 mb-2">
          <MapPin size={12} color={colors.mutedForeground} style={{ marginTop: 1 }} />
          {/* cardAddressStyle (flexWrap:'wrap') is correct here — full address should be readable */}
          <Text className="text-xs text-muted-foreground flex-1" style={[{ fontFamily: Fonts.body }, cardAddressStyle]}>
            {c.address}
          </Text>
        </View>
      )}
      ...
      <View className="flex-row border-t border-border/40 pt-2">
        <PermissionGuard module="Customers" action="Update">
          <TouchableOpacity onPress={() => openEdit(c)} className="flex-1 py-2 flex-row items-center justify-center gap-2 border-r border-border/40">
            <Pencil size={13} color={colors.primary} />
            <Text className="text-xs font-bold text-primary" style={{ fontFamily: Fonts.body }}>Edit</Text>
          </TouchableOpacity>
        </PermissionGuard>
        <PermissionGuard module="Customers" action="Delete">
          <TouchableOpacity onPress={() => setDeleteId(c.id)} className="flex-1 py-2 flex-row items-center justify-center gap-2">
            <Trash2 size={13} color={colors.destructive} />
            <Text className="text-xs font-bold text-destructive" style={{ fontFamily: Fonts.body }}>Delete</Text>
          </TouchableOpacity>
        </PermissionGuard>
      </View>
    </View>
  ...
  return (
    <MasterScreenLayout title="Customers" subtitle="Manage customer directory" module="Customers" onAddNew={openAdd} addNewLabel="Add Customer">
      <AdaptiveTable
        data={data}
        columns={columns}
        loading={isLoading}
        ...
        filters={[
          { key: 'phone', label: 'Phone', options: [] },
        ]}
        filterValues={{ phone: '' }}
        onFilterChange={() => { }}
        selectedIds={selectedIds}
        onSelectAll={(a) => setSelectedIds(a ? new Set(data.map(d => d.id)) : new Set())}
        onSelectRow={toggleSelect}
        onBulkDelete={selectedIds.size > 0 ? () => setBulkDeleteOpen(true) : undefined}
        exportTitle="Customers Report"
        ...
        showImport
        onImport={(rows) => { rows.forEach(r => save.mutate({ payload: r })); }}
      ...
      <FormModal
        ...
        footer={
          <View className="flex-row gap-3">
            <TouchableOpacity onPress={() => setFormOpen(false)} className="flex-1 h-11 rounded-xl border border-border items-center justify-center">
              <Text className="text-sm font-bold" style={{ fontFamily: Fonts.body }}>Cancel</Text>
            </TouchableOpacity>
            <TouchableOpacity
              onPress={() => save.mutate({ id: editItem?.id, payload: form }, { onSuccess: () => setFormOpen(false) })}
              disabled={save.isPending}
              className="flex-1 h-11 rounded-xl bg-primary items-center justify-center"
            >
              <Text className="text-sm font-bold text-primary-foreground" style={{ fontFamily: Fonts.body }}>
      ...
      <DeleteConfirmModal
        ...
        itemName="customer"
        onConfirm={() => bulkRemove.mutate([...selectedIds], { onSuccess: () => { setSelectedIds(new Set()); setBulkDeleteOpen(false); } })}
